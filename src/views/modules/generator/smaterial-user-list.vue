  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('generator:smaterial:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column
        header-align="center"
        align="center"
        label="文件预览" min-width="300px;">
        <template slot-scope="scope">
          <img :src="itemUrl" v-for="(itemUrl,index) in scope.row.urls" @click="showPic(itemUrl)" style="float: left;display: inline-block; width: 100px;margin:10px;"/>
        </template>
      </el-table-column>
      <el-table-column
        prop="preAudit"
...
<script>
  import AddOrUpdate from './smaterial-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    activated () {
      if(this.$route.params.userId && this.$route.params.userName) {
        if(this.$route.params.userId){
          this.dataForm.userId = this.$route.params.userId;
        }
        if(this.$route.params.userName)
        {
          this.dataForm.userName = this.$route.params.userName;
        }
        this.getDataList()
      }
      else
    ...
    methods: {
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/generator/smaterial/delete'),
            method: 'post',
